...
import { AIRTABLE_API_KEY, AIRTABLE_BASE_ID, AIRTABLE_SERVICES_FEEDBACK_TABLE_ID } from 'react-native-dotenv';
...
const getRequestUrl = (): string => {
    ...
    if (!AIRTABLE_SERVICES_FEEDBACK_TABLE_ID) {
        throw new Error('AIRTABLE_SERVICES_FEEDBACK_TABLE_ID is missing.');
    }

    return buildUrl('https://api.airtable.com', { path: `v0/${AIRTABLE_BASE_ID}/${AIRTABLE_SERVICES_FEEDBACK_TABLE_ID}` });
};
